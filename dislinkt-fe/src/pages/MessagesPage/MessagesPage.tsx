...
import { getConversations, startConversation, sendMessage, initWs, terminateWs } from "../../store/actions/message-actions";
import { getNotBlockedProfiles, getProfile } from "../../store/actions/profile-actions";
...
const MessagesPage: React.FC = () => {
  ...
  const displayedConversation: ConversationData | undefined = useSelector((state: RootState) => state.messages.displyedConversation);

  const profiles: ProfileSimple[] = useSelector(
    (state: RootState) => state.profile.profiles.filter(profile => profile.id != state.auth.userData.id)
  );
  ...
  const userId: string = useSelector((state: RootState) => state.auth.userData.id);

  const user: ProfileNestedData = useSelector(
    (state: RootState) => ({
  ...
  const handleSendMessage = () => {
    ...
    if (displayedConversation.id == TEMP_CONVESATION) {
      dispatch(startConversation(user, displayedConversation.participants.find(x => x.globalId != userId)!, text))
    } else (
      dispatch(sendMessage(displayedConversation.id, userId, text))
    )
      ...
      <div className={classes["layout"]}>
        {
          !displayedConversation ?
            ...
            : <Chat
              ...
              name={displayedConversation.participants.find(x => x.globalId != userId)?.name || ""}
              surname={displayedConversation.participants.find(x => x.globalId != userId)?.surname || ""}
              picture={displayedConversation.participants.find(x => x.globalId != userId)?.avatar || ""}
        ...
        <div className={classes["side"]}>
          <ChatSuggestions profiles={profiles} profileSelected={startConversationWith} />
          <ChatList userId={userId} conversations={conversations} conversationSelected={conversationSelected} />
        </div>
